...
```
출력은 부문별 수집 건수·목표 대비 표다. **`[ALERT] 수집 0건 부문`이 뜨면 그냥 넘어가지 마라.**
- 해당 부문의 쿼리를 **한 번 더 시도**한다(Exa 응답이 일시적으로 비는 경우가 있다).
- 재시도 후에도 0건이면 그대로 진행하되, **Step 9 보고에 어느 부문이 왜 비었는지 반드시 적는다.**
...
작성하라(600~700단어 한국어). 수치·핵심 키워드·중요 판단은 **굵게**(`**text**`).
